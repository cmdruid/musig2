...
export default function (t : Test) {
  ...
  const key_ctx = musig.ctx.from_pubkeys(group_keys, options)
  ...
  for (const message of messages) {
    ...
    const signature = musig.combine.psigs(ctx, group_psigs)

    // Check if the signature is valid.
    sig_results.push(musig.verify.with_ctx(ctx, signature))
